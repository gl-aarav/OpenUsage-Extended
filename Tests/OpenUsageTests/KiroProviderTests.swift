...
final class KiroProviderTests: XCTestCase {
    ...
    func testRefreshFetchesUsageWithTokenFile() async throws {
        let tokenJSON = try JSONSerialization.data(withJSONObject: [
            "accessToken": "FAKE_KIRO_ACCESS_TOKEN",
            "refreshToken": "FAKE_KIRO_REFRESH_TOKEN",
            "profileArn": "arn:aws:codewhisperer:us-east-1:000000000000:profile/FakeProfile",
            "region": "us-east-1"
        ])
    ...
    func testRefreshRetriesAfterTokenRefresh() async throws {
        ...
        XCTAssertEqual(httpClient.requests[0].headers["Authorization"], "Bearer FAKE_EXPIRED_TOKEN")
        XCTAssertEqual(httpClient.requests[2].headers["Authorization"], "Bearer FAKE_FRESH_TOKEN")
    }
